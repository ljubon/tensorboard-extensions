...
def gr_tensorboard_wsgi(flags, plugin_loaders, assets_zip_provider):
    ...
    run_path_map = _getRunPathMapFromLogdir(flags.logdir,
                                            flags.enable_first_N_runs)
    _logger.log_message_info(
        "loading EventMultiplexer with the %d most recent runs enabled by default"
        % flags.enable_first_N_runs)
    gr_multiplexer = plugin_event_multiplexer.EventMultiplexer(
        run_path_map=run_path_map,
    ...
    _logger.log_message_info(
        "Done loading all plugins, now launching the tensorboard application")
...
def _getRunPathMapFromLogdir(logdir, most_recent_num):
    if most_recent_num == 0:
        ...
        dir_list = sorted(io_helpers.get_run_paths(logdir),
                          key=os.path.getmtime)
        num_files = min(most_recent_num, len(dir_list))
        return {
            os.path.relpath(path, logdir): path
            for path in dir_list[-num_files:]
        }
    else:
        return {
            os.path.relpath(path, logdir): path
            for path in io_helpers.get_run_paths(logdir)
        }
