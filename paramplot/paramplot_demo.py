...
def main(unused_argv):
    ...
    parameters = [(x, y, z, u) for x in int_parameters
                  for y in float_parameters for z in float2_parameters
                  for u in float3_parameters]
    ...
    def create_parameters(int_param, float_param, float2_param,
                          param2electricboogaloo):
    ...
    tag_value_maps = {
        runs[index]: create_metrics(index)
        for (index, _) in enumerate(parameters)
    }

    parameter_maps = {
        runs[index]: create_parameters(*params)
